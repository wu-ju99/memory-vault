...
- 用户注册/登录（JWT 鉴权）
- 用户头像会显示在相册、媒体、评论、用户分区和管理员面板中，未设置头像时使用显示名首字母兜底
- 相册创建与管理（支持用户选择相册年份）
- 首页按相册年份分区，年份内再按相册创建者分区，并支持用户子导航定位
- 独立管理员面板（成员、相册、媒体管理）
- 图片/视频批量上传（10MB / 500MB 限制）
- 媒体描述编辑、删除
- 相册详情按年份分区，年份内再按上传者分区，单个用户内继续按图片/视频分区展示
...
| 登录 | `/login` | 用户名/密码登录表单 |
| 注册 | `/register` | 创建新账号 |
| 首页 | `/` | 相册列表 + 新建相册 + 年份导航 + 年内用户分区 |
| 相册详情 | `/album/:id` | 上传 + 年份分区 + 年内上传者分区 + 图片/视频分区 + 评论 |
