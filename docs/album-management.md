...
- 首页当前按相册年份主分区，年份内再按相册创建者分区；单个用户内按创建时间倒序展示相册。
...
- `useAlbumYears` 负责按 `album_year` 分组和年份定位。
- `useAlbumUsers` 负责年份内按相册创建者分组；单个用户内按创建时间倒序。
- `useYearUserNav` 负责年份导航下方的用户子导航数据。
- `Home.jsx` 负责页面组织、确认框和把操作传给组件。
- `AlbumYearNav.jsx` / `AlbumYearSection.jsx` 负责年份导航和年份主分区展示。
- `AlbumUserNav.jsx` / `AlbumUserSection.jsx` 负责用户子导航和年内用户相册分区展示。
