...
async function getDisplayById(id) {
  const [rows] = await pool.query(
    'SELECT a.id, a.user_id, a.title, a.album_year, a.cover_url, a.created_at, u.username, u.nickname, u.avatar, u.role FROM albums a JOIN users u ON a.user_id = u.id WHERE a.id = ?',
    [id]
  );
  return rows[0] || null;
}
