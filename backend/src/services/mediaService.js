...
async function getList(userId, albumId) {
  let sql = 'SELECT m.id, m.user_id, m.album_id, m.url, m.type, m.size, m.description, m.event_time, m.created_at, a.title AS album_title, u.username, u.nickname, u.role FROM media m LEFT JOIN albums a ON m.album_id = a.id JOIN users u ON m.user_id = u.id';
  const params = [];
